...
# SERVER ################################################################# -----
followupreport_Server <- function(input, output, session, data_i, dataset, dataset_variables,
  ...
  observe({
    ...
    dataset <- bind_rows(dataset, data_i) %>%
      ...
      distinct(uid, report_type, followup_report_id, .keep_all = TRUE)
    ...
    manualgen_followup_report <- function(data_i){
      fluidRow(
        fluidRow(
          column(4, data_i$fu_row1_c1),
          column(4, data_i$fu_row1_c2),
          column(4, data_i$fu_row1_c3)
        ),
        fluidRow(column(12,data_i$fu_row2)),
        fluidRow(column(12,data_i$fu_row3)),
        fluidRow(column(12,data_i$fu_row4)),
        fluidRow(column(12,data_i$fu_row5)),
        fluidRow(
          column(4, data_i$fu_row6_c1),
          column(4, data_i$fu_row6_c2),
          column(4, data_i$fu_row6_c3)
        ),
        br(),
        fluidRow(column(12, data_i$fu_main_text)),
        br(),
        fluidRow(column(12, data_i$fu_row7)),
        fluidRow(column(12, data_i$fu_row8)),
        fluidRow(column(12, data_i$fu_row9)),
        fluidRow(column(12, data_i$fu_row10))
      )
    }
    
    output$fu_report_manual_v1 <- renderUI(manualgen_followup_report(data_i))
    output$fu_report_manual_v2 <- renderUI(manualgen_followup_report(data_i))
    output$fu_report_manual_v3 <- renderUI(manualgen_followup_report(data_i))
    output$fu_report_manual_v4 <- renderUI(manualgen_followup_report(data_i))
    
    # ** Required Variables ----------------------------------------------------
    
    # **** Allow Feedback/Finalize -----
    allow_feedback_finalize <- !("" %in% input$officer_fu_feedback_title) &
      !("" %in% input$officer_fu_feedback_firstname) & 
      !("" %in% input$officer_fu_feedback_surname)
  ...
  # DOWNLOAD HANDLER ********************* -------------------------------------
  output$fu_pdf = downloadHandler(
    filename = paste0('followup_report_OB',data_i$ob_no,'.pdf'),
    
    content = function(file) {
      out = knit2pdf(file.path('modules_gen_reports', 'generate_followup_pdf_parent.Rnw'), clean = TRUE)
  ...
  gen_fu_tabs <- function(tab_type){
    renderUI({
      ...
      div(
        ...
        if(tab_type %in% "Enter Data"){
        ...
        } else if(tab_type %in% "Write Follow Up Report"){
          
          fluidRow(
            column(6,
                   uiOutput(session$ns("edit_followup_report_ui"))
            ),
            column(6,
                   fluidRow(
                     column(12,
                            h4("Follow Up Report", align = "center")
                     ),
                     column(12,
                            uiOutput(session$ns("fu_report_manual_v1"))
                     )
                   )
            )
          )
          ...
          fluidRow(
            column(6,
                   ...
                   fluidRow(
                     column(12, align = "center",
                            textAreaInput(session$ns("fu_station_comments"), "If needed, provide comments for HQ"),
                     )
                   ),
                   fluidRow(
                   ...
                   fluidRow(
                     column(3, textInput(session$ns("station_fu_feedback_title"), "Title", "")),
                     column(3, textInput(session$ns("station_fu_feedback_firstname"), "First Name", "")),
                     column(3, textInput(session$ns("station_fu_feedback_surname"), "Surname", "")),
                     column(3, textInput(session$ns("station_fu_feedback_snumber"), "Service No.", ""))
                   ),
                   br(),
                   br(),
                   br(),
            ...
            column(6,
                   uiOutput(session$ns("fu_report_manual_v2")))
          )
          
          # ** Review Sit Report - hq ------------------------------------------
        } else if(tab_type %in% "Review Follow Up Report" & USER_ROLE %in% "hq"){
          fluidRow(
            column(6,
                   
                   fluidRow(
                     column(12,  offset = 2, align = "left",
                            htmlOutput(session$ns("fu_station_comments_txt")),
                     )
                   ),
                   br(),
                   fluidRow(
                     column(12,  offset = 2, align = "left",
                            htmlOutput(session$ns("station_fu_feedback_title_name_txt")),
                     )
                   ),
                   ...
                   fluidRow(
                     column(12, align = "center",
                            textAreaInput(session$ns("sr_hq_comments"), "If needed, provide comments for changes to the follow up report."),
                     )
                   ),
                   fluidRow(
                     column(12, align = "center", strong("Title and Name")),
                   ...
                   fluidRow(
                     column(3, textInput(session$ns("officer_fu_feedback_title"), "Title", "")),
                     column(3, textInput(session$ns("officer_fu_feedback_firstname"), "First Name", "")),
                     column(3, textInput(session$ns("officer_fu_feedback_surname"), "Surname", "")),
                     column(3, textInput(session$ns("officer_fu_feedback_snumber"), "Service No.", ""))
                   ),
            ...
            column(6,
                   uiOutput(session$ns("fu_report_manual_v3")))
          )
          # ** Review Sit Report - officer -------------------------------------
        } else if(tab_type %in% "Review Follow Up Report" & USER_ROLE %in% "officer"){
          fluidRow(
            column(6, align = "center",
                   h3("Submitted to HQ"),
                   fluidRow(
                     column(12, align = "center",
                            strong("The follow up report has been submitted to HQ 
          ...
          fluidRow(
            column(6, align = "center",
                   ...
                   column(8,  offset = 2, align = "left",
                          textOutput(session$ns("fu_hq_comments_txt")),
                   )
            ),
            column(6,
                   uiOutput(session$ns("fu_report_manual_v4"))
            )
          )
  ...
  output$fu_tabs_ui <- renderUI({
    do.call(tabsetPanel,
            lapply(tab_types,
                   function(i) tabPanel(i, gen_fu_tabs(i) )))
  })
  ...
  output$edit_followup_report_ui <- renderUI({
    
    div(
      fluidRow(
        column(4,
               textInput(session$ns("fu_row1_c1"), NULL, data_i$fu_row1_c1)
        ),
        column(4,
               textInput(session$ns("fu_row1_c2"), NULL, data_i$fu_row1_c2)
        ),
        column(4,
               textInput(session$ns("fu_row1_c3"), NULL, data_i$fu_row1_c3)
        )
      ),
      fluidRow(column(12, textInput(session$ns("fu_row2"), NULL, data_i$fu_row2, width = "120%"))),
      fluidRow(column(12, textAreaInput(session$ns("fu_row3"), NULL, data_i$fu_row3, width="130%", height = 100))),
      fluidRow(column(12, textInput(session$ns("fu_row4"), NULL, data_i$fu_row4, width = "120%"))),
      fluidRow(column(12, textInput(session$ns("fu_row5"), NULL, data_i$fu_row5, width = "120%"))),
      fluidRow(
        column(4,
               textInput(session$ns("fu_row6_c1"), NULL, data_i$fu_row6_c1)
        ),
        column(4,
               textInput(session$ns("fu_row6_c2"), NULL, data_i$fu_row6_c2)
        ),
        column(4,
               textInput(session$ns("fu_row6_c3"), NULL, data_i$fu_row6_c3)
        )
      ),
      fluidRow(column(12, textAreaInput(session$ns("fu_main_text"), NULL,  data_i$fu_main_text,
                                        width="130%", height=200))),
      fluidRow(column(12, textInput(session$ns("fu_row7"), NULL, data_i$fu_row7, width = "120%"))),
      fluidRow(column(12, textInput(session$ns("fu_row8"), NULL, data_i$fu_row8, width = "120%"))),
      fluidRow(column(12, textInput(session$ns("fu_row9"), NULL, data_i$fu_row9, width = "120%"))),
      fluidRow(column(12, textInput(session$ns("fu_row10"), NULL, data_i$fu_row10, width = "120%")))
  ...
  observeEvent(input$submit_to_hq_yes, { 
    ...
    # ** Change Status - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    data_i$stage_fu <- "submitted_to_hq"
    data_i$stage_fu_num <- as.numeric(data_i$stage_fu_num) + 1
    ...
    update_p69_r$value <<- 10
    update_summary_table_r$value <<- 10
    sync_server_trigger_r$value <<- 25
  ...
  output$fu_station_comments_txt <- renderText({
    
    out <- ""
    
    if(!is.null(data_i$fu_station_comments)){
      if(!is.na(data_i$fu_station_comments)){
        if(nchar(data_i$fu_station_comments) > 1){
          
          out <- paste("<h3>Message from Officer:</h3>", 
                       data_i$fu_station_comments)
  ...
  output$station_fu_feedback_title_name_txt <- renderText({
      ...
      if(!is.na(data_i$fu_station_comments)){
        if(nchar(data_i$fu_station_comments) > 1){
          ...
          out <- paste("<strong>Message from:</strong>", 
                       data_i$station_fu_feedback_title, 
                       data_i$station_fu_feedback_firstname, 
                       data_i$station_fu_feedback_surname,
                       "<br>", 
                       "<strong>Service Number:</strong>", data_i$station_fu_feedback_snumber,
                       "<hr>")
  ...
  observeEvent(input$submit_for_feedback,{
    ...
    # ** Change Status - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    data_i$stage_fu <- "hq_provided_feedback"
    data_i$stage_fu_num <- as.numeric(data_i$stage_fu_num) + 1
    ...
    update_p69_r$value <<- 10
    update_summary_table_r$value <<- 10
    sync_server_trigger_r$value <<- 61
    ...
    shinyalert(title = ifelse(curl::has_internet(), 
               ...
               text = ifelse(curl::has_internet(),
                             "The follow up report has been sent back to the station for edits.",
  ...
  observeEvent(input$finalize,{
    ...
    update_p69_r$value <<- 10
    update_summary_table_r$value <<- 10
    sync_server_trigger_r$value <<- 93

    removeModal() 
    
    shinyalert(title = "Report Finalized",
               text = ifelse(curl::has_internet(),
                             "The follow up report has been finalized.",
                             "The follow up report has been finalized. For all stations to see the follow up report as finalized, connect to internet and click the \"Sync Data with Server\" button."),
               type = "success")
